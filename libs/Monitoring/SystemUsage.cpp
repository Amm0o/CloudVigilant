...
MemoryInfo SystemUsage::getMemoryUsage()
{
    ...
    while (std::getline(file, line))
    {
        if (line.find("MemTotal:") == 0)
            parseLine(line, totalMemory);
        else if (line.find("MemFree:") == 0)
            parseLine(line, freeMemory);
        else if (line.find("Buffers:") == 0)
            parseLine(line, bufferMemory);
        else if (line.find("Cached:") == 0)
            parseLine(line, cachedMemory);
...
// Tracker for disk
DiskInfo SystemUsage::getDiskUsage()
{
    // Struct to hold disk info coming from statvfs
    struct statvfs stat;

    // Get disk usage info
    if (statvfs("/", &stat) != 0)
    {
        std::cerr << "Error getting disk usage for /" << std::endl;
        return {-1, -1, -1};
    }
    ...
    diskInfo.total = (stat.f_blocks * stat.f_frsize) / 1024 / 1024 / 1024;
    diskInfo.free = (stat.f_bavail * stat.f_frsize) / 1024 / 1024 / 1024;
